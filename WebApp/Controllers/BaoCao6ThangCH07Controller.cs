...
{
    public class BaoCao6ThangCH07Controller : BaseController
    {
        ...
        public BaoCao6ThangCH07Controller(ICityDA cityDA, IDuAnDA duAnDA, IBaoCaoTongHopDA baoCaoTongHopDA, IBVTL_NHOM_TBHDA bVTL_NHOM_TBHDA, ISysLogDA sysLogDA)
        {
            _CityDA = cityDA;
            _DuAnDA = duAnDA;
        ...
        // GET: BaoCao6ThangCH07
        [HasCredential(ControllerName = "BaoCao6ThangCH07")]
                    ...
                    if (data.Any())
                    {
                        ...
                        foreach (var rowReport in data)
                        {
                            if (rowReport.IsShow == "Y")
                            {
                                ...
                                InsertDataCell(ws, "E", row, rowReport.Nam > 0 ? rowReport.Nam.ToString() : "", true, XLAlignmentHorizontalValues.Right, XLAlignmentVerticalValues.Center, rowReport.Nam > 0 ? true : false);

                                // Thêm dữ liệu cột Nữ
                                InsertDataCell(ws, "F", row, rowReport.Nu > 0 ? rowReport.Nu.ToString() : "", true, XLAlignmentHorizontalValues.Right, XLAlignmentVerticalValues.Center, rowReport.Nu > 0 ? true : false);
        ...
        private void CreateFooter(IXLWorksheet ws, string tileReport, UserLogin user, string tenNhomTBHs)
        {
            ...
            ws.Cell("F71").Value = "Quản lý chương trình/Điều phối";
