...
{
    public class PhieuTuVanController : BaseController
    {
        readonly IPhieuTuVanDA _PhieuTuVanDA;
        readonly ISysLogDA _sysLogDA;
        readonly IDuAnDA _DuAnDA;
        BaseController _helperController = new BaseController();
        readonly ICityDA _CityDA;

        public PhieuTuVanController(IPhieuTuVanDA PhieuTuVanDA, ISysLogDA sysLogDA, IDuAnDA DuAnDA, ICityDA CityDA)
        {
            _PhieuTuVanDA = PhieuTuVanDA;
            _sysLogDA = sysLogDA;
            _DuAnDA = DuAnDA;
            _CityDA = CityDA;
        }
        ...
        // GET: PhieuTuVan
        [HasCredential(ControllerName = "PhieuTuVan")]
        public ActionResult Index()
        {
            try
            {
                ...
                if (user.IsAdmin || (duAn != null && user.MaDuAn.Contains(duAn.maduan)))
                {
                    ...
                    var data = _PhieuTuVanDA.GetAllByPage(modelSearch);
                    return View(data);
                }
                else
        ...
        public ActionResult GetAll(ModelSearch modelSearch)
        {
            ...
            try
            {
                ...
                AddLog("Lấy dữ liệu theo trang bảng Phiếu tư vấn( keyword: " + modelSearch.KeyWord + ", page: " + modelSearch.currentPage + ") thành công.");
            ...
            catch (Exception ex)
            {
                ...
                AddLog("Lấy dữ liệu theo trang bảng Phiếu tư vấn( keyword: " + modelSearch.KeyWord + ", page: " + modelSearch.currentPage + ") lỗi: " + ex.Message);
        ...
        public ActionResult GetBottomAction()
        {
            ...
            try
            {
                ...
                AddLog("Lấy danh sách các botom được thực hiện trên from Phiếu tư vấn thành công.");
            ...
            catch (Exception ex)
            {
                obj.Error = true;
                obj.Title = ex.Message.ToString();
                AddLog("Lấy danh sách các botom được thực hiện trên from Phiếu tư vấn lỗi: " + ex.Message);
                return Json(obj);
            }
        }
        ...
        public object GetItemByID(int Id)
        {
            try
            {
                var data = _PhieuTuVanDA.GetItemById(Id);
                AddLog("Lấy dữ liệu theo ID bảng Phiếu tư vấn( ID: " + Id + ") thành công.");
                return Json(new { Error = false, Title = "Lấy dữ liệu thành công.", data = data });
            }
            catch (Exception ex)
            {
                AddLog("Lấy dữ liệu theo ID bảng Phiếu tư vấn( ID: " + Id + ") lỗi: " + ex.Message);
        ...
        public ActionResult ExportData(string keyword)
        {
            try
            {
                ...
                string file_name = "PhieuTuVan_" + DateTime.Now.ToShortDateString() + "_" + DateTime.Now.ToShortTimeString() + ".xlsx";
                ...
                {
                    var ws = wb.Worksheets.Add("Phiếu tư vấn");
                    ws.Cell("A2").Value = "Phiếu tư vấn";
            ...
            {
                AddLog("Export Phiếu tư vấn lỗi: " + ex.Message);
