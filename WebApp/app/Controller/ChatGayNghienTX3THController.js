app.controller("ChatGayNghienTX3THController", function ($scope, $uibModal, $ngConfirm, showToast, hideLoading) {
    $scope.modelSearch = {};
    ...
    var dataTableChatGayNghienTX3TH = null;
    ...
    function GetBottomAction() {
        $.ajax({
            type: 'post',
            url: '/ChatGayNghienTX3TH/GetBottomAction',
            data: {},
    ...
    $('#dataTableChatGayNghienTX3TH').on('click', 'tr', function () {
    ...
    $scope.LoadPage = function (genTable) {
        ...
        $scope.ListChatGayNghienTX3TH = [];
        if (genTable == 1 || genTable == 2) {
            if (genTable == 2)
                dataTableChatGayNghienTX3TH.destroy();
            dataTableChatGayNghienTX3TH = $('#dataTableChatGayNghienTX3TH').DataTable({
                ...
                ajax: function (data, callback, settings) {
                    ...
                    $.ajax({
                        ...
                        success: function (respone) {
                            ...
                            $scope.ListChatGayNghienTX3TH = respone.data;
                            if (respone.data != null && respone.data.length > 0) {
                                for (var i = 0; i < respone.data.length; i++) {
                                    var tmp = {
                                        STT: i + 1,
                                        id: respone.data[i].id,
                                        ten_cgn: respone.data[i].ten_cgn,
            ...
            dataTableChatGayNghienTX3TH.ajax.reload();
        }
        hideLoading();
    };
    ...
    $scope.edit = function () {
        var seletedRow = dataTableChatGayNghienTX3TH.rows({ selected: true });
        ...
        if ($scope.ParamIdSeleted > 0 && $scope.ParamIdSeleted != undefined) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: '/ChatGayNghienTX3TH/_Edit',
                controller: 'edit',
                size: 'xl',
                backdrop: 'static',
                resolve: {
