app.controller("QSTangGiamTrongKyController", function ($scope, $uibModal, $ngConfirm, showToast, hideLoading, $location) {
    //#region Khởi tạo
    $scope.treeLucLuong;
    $scope.treeCapBac;
    ...
    $scope.ChangeNhomCapBac = function () {
        if ($scope.treeCapBac !== undefined)
            $scope.CapBacId = $scope.treeCapBac._selectedItem !== undefined ? $scope.treeCapBac._selectedItem.id : null;
    }
    ...
    function onClickTCCBCS(event, treeId, treeNode, clickFlag) {
        ...
        if (treeNode !== undefined && treeNode.id !== undefined) {
            ...
            $scope.LoadDataTangGiamTrongKy($scope.DON_VI_ID);
    ...
    //#region Quân số tăng giảm trong kỳ
    $scope.SearhData = function () {
        $scope.LoadDataTangGiamTrongKy($scope.DON_VI_ID);
    }

    $scope.LoadDataTangGiamTrongKy = function (donViId) {
        $.ajax({
            type: 'post',
            url: '/QSTangGiamTrongKy/GetDanhSach',
