app.controller("XuatKhoController", function ($scope, $uibModal, $ngConfirm, showToast, hideLoading, chuyenGiaSangChu) {
    $scope.canbo = [
    ...
    $scope.ShowThongTin = (item, index) => {
        ...
        $scope.thongTinKho.NGUOI_GIAO = $scope.canbo.find(x => x.ID === $scope.thongTinKho.ID_NGUOI_GIAO);
        ...
        $scope.thongTinKho.NGUOI_KY_5 = $scope.canbo.find(x => x.ID === $scope.thongTinKho.ID_NGUOI_KY_5);
        ...
        if ($scope.thongTinKho.NGAY_XK != null) {
            if ($scope.thongTinKho.NGAY_XK.length === 8) {
                $scope.thongTinKho.NGAY_XK_VW = moment($scope.thongTinKho.NGAY_XK, 'YYYYMMDD').format('DD/MM/YYYY') 
            }
        }
    ...
    $scope.LuuPhieu = () => {
        ...
        try {
            if (CheckThongTin()) {
                $scope.thongTinKho.NGAY_XK = moment($scope.thongTinKho.NGAY_XK_VW, 'DD/MM/YYYY').format('YYYYMMDD');
                ...
                $scope.thongTinKho.ID_NGUOI_KY_5 = $scope.thongTinKho.NGUOI_KY_5.ID;
                $scope.thongTinKho.TEN_NGUOI_KY_5 = $scope.thongTinKho.NGUOI_KY_5.HoTen;
                $scope.thongTinKho.CV_NGUOI_KY_5 = $scope.thongTinKho.NGUOI_KY_5.ChucVu;
                $scope.thongTinKho.CB_NGUOI_KY_5 = $scope.thongTinKho.NGUOI_KY_5.CapBac;
                ...
                if ($scope.dsXuatKho.length === 0) {
                    ...
                    $scope.dsXuatKho.forEach((x) => { x.ID_HANG = x.HANG_HOA.ID; x.MA_HANG = x.HANG_HOA.MaHang; x.TEN_HANG = x.HANG_HOA.TenHang, x.DVT = x.DVT_VW.NAME, x.ID_NK_CT = x.ID_XK_CT });
                    $.ajax({
                        type: 'post',
                        url: '/XuatKho/UpdateXuatKho',
                        data: {
    ...
    CheckThongTin = () => {
        if ($scope.thongTinKho.NGAY_XK_VW == null || $scope.thongTinKho.NGUOI_KY_4 == null || $scope.thongTinKho.NGUOI_KY_5 == null || $scope.thongTinKho.NGUOI_PHU_TRACH == null || $scope.thongTinKho.NGUOI_GIAO == null || $scope.thongTinKho.NGUOI_NHAN == null) {
            toastr.error("Chưa nhập đủ thông tin");
            return false;
        }
        //if ($scope.dsNhapKho.some(element => CheckDsHangHoa(element))) {
    ...
    $scope.XacNhan = () => {
        ...
        try {
            $.ajax({
                type: 'post',
                url: '/TonKho/InsertSoLuongTonKho',
                data: {
                    idKho: $scope.thongTinKho.ID_KHO,
                    lstIdHang: $scope.dsXuatKho,
                    loaiPhieu: 'X',
                    soPhieu: $scope.thongTinKho.SO_PHIEU,
                    ngayCt: $scope.thongTinKho.NGAY_CT
                },
                success: function (response) {
                    if (response.Status === 200) {
            ...
            toastr.error('Có lỗi xảy ra trong quá trình lưu!' + e);
            console.error(e);
        }
        finally {
            hideLoading();
    ...
    $scope.HuyPhieu = () => {
        ...
        try {
            $.ajax({
                type: 'post',
                url: '/TonKho/HuyChungTuXuatNhapDc',
                data: {
                    idKho: $scope.thongTinKho.ID_KHO,
                    lstIdHang: $scope.dsNhapKho,
                    soPhieu: $scope.thongTinKho.SO_PHIEU,
                    loaiPhieu: 'X'
                },
                success: function (response) {
                    if (response.Status === 200) {
                        toastr.success('Hủy phiếu xuất thành công!');
                    }
                    else {
                        toastr.error(response.Message);
                    }
    ...
    $scope.GetAnhPhieu1 = () => {
        if ($scope.thongTinKho.IMAGE1 == null && $scope.thongTinKho.IMG_PATH_1 != null) {
            $.ajax({
                type: 'post',
                url: '/XuatKho/GetImageFTP',
                data: {
                    path: $scope.thongTinKho.IMG_PATH_1
                },
                success: function (response) {
                    if (response.Status === 200) {
    ...
    $scope.GetAnhPhieu2 = () => {
        if ($scope.thongTinKho.IMAGE2 == null && $scope.thongTinKho.IMG_PATH_2 != null) {
            $.ajax({
                ...
                url: '/XuatKho/GetImageFTP',
                data: {
                    path: $scope.thongTinKho.IMG_PATH_2
                },
                success: function (response) {
                    if (response.Status === 200) {
    ...
    $scope.GetAnhPhieu3 = () => {
        if ($scope.thongTinKho.IMAGE3 == null && $scope.thongTinKho.IMG_PATH_3 != null) {
            $.ajax({
                ...
                url: '/XuatKho/GetImageFTP',
                data: {
                    path: $scope.thongTinKho.IMG_PATH_3
                },
                success: function (response) {
                    if (response.Status === 200) {
    ...
    $scope.ValueCount = (item, index) => {
        if (typeof item.SO_LUONG_TT == 'number' && typeof item.DON_GIA == 'number') {
            $scope.dsXuatKho[index].THANH_TIEN_TT = item.SO_LUONG_TT * item.DON_GIA;
        };
    }
});
