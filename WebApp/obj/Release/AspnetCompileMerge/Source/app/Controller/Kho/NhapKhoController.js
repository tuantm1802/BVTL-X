app.controller("NhapKhoController", function ($scope, $uibModal, $ngConfirm, showToast, hideLoading) {
    ...
    $scope.ShowThongTin = (item, index) => {
        ...
        try {
            ...
            if ($scope.thongTinKho.NGAY_NK != null) {
                if ($scope.thongTinKho.NGAY_NK.length === 8) {
                    $scope.thongTinKho.NGAY_NK_VW = moment($scope.thongTinKho.NGAY_NK, 'YYYYMMDD').format('DD/MM/YYYY')
                }
            }
    ...
    $scope.LuuPhieu = () => {
        ...
        try {
            if (CheckThongTin()) {
                $scope.thongTinKho.NGAY_NK = moment($scope.thongTinKho.NGAY_NK_VW, 'DD/MM/YYYY').format('YYYYMMDD');
                ...
                if ($scope.dsNhapKho.length === 0) {
                    ...
                    $scope.dsNhapKho.forEach((x) => { x.ID_HANG = x.HANG_HOA.ID; x.MA_HANG = x.HANG_HOA.MaHang; x.TEN_HANG = x.HANG_HOA.TenHang, x.DVT = x.DVT_VW.NAME, x.ID_NK_CT = x.ID_NK_CT });
                    $.ajax({
                        type: 'post',
                        url: '/NhapKho/UpdateNhapKho',
                        data: {
        ...
        try {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    toastr.error('Có lỗi xảy ra trong quá trình xác nhận!');
                }
            });
        } catch (e) {
            toastr.error('Có lỗi xảy ra trong quá trình xác nhận!' + e);
            console.error(e);
        }
        finally {
            hideLoading();
    ...
    $scope.GetAnhPhieu1 = () => {
        if ($scope.thongTinKho.IMAGE1 == null && $scope.thongTinKho.IMG_PATH_1 != null) {
            $.ajax({
                type: 'post',
                url: '/DinhKem/GetImageDinhKem',
                data: {
                    imgPath: $scope.thongTinKho.IMG_PATH_1
                },
                success: function (response) {
                    if (response.Status === 200) {
                        ...
                        toastr.error('Có lỗi xảy ra trong quá trình lấy ảnh!');
                    }
                    console.log(response);
                    $scope.$apply();
                }, error: (e) => {
    ...
    $scope.GetAnhPhieu2 = () => {
        if ($scope.thongTinKho.IMAGE2 == null && $scope.thongTinKho.IMG_PATH_2 != null) {
            $.ajax({
                ...
                url: '/DinhKem/GetImageDinhKem',
                data: {
                    imgPath: $scope.thongTinKho.IMG_PATH_2
                },
                success: function (response) {
                    if (response.Status === 200) {
    ...
    $scope.GetAnhPhieu3 = () => {
        if ($scope.thongTinKho.IMAGE3 == null && $scope.thongTinKho.IMG_PATH_3 != null) {
            $.ajax({
                ...
                url: '/DinhKem/GetImageDinhKem',
                data: {
                    imgPath: $scope.thongTinKho.IMG_PATH_3
                },
                success: function (response) {
                    if (response.Status === 200) {
    ...
    $scope.imageIsLoaded = function (e) {
        $scope.$apply(function () {
            ...
            //    if (i === 0) {
            //        $scope.thongTinKho.IMAGE1 = $scope.stepsModel[0].split(',')[1];
            //        $('#hiddenImg1').attr('src', 'data: image / jpeg; base64,' + $scope.thongTinKho.IMAGE1);
            //        $scope.CoDinhKem1 = true;
            //    }
            //    if (i === 1) {
            //        $scope.thongTinKho.IMAGE2 = $scope.stepsModel[1].split(',')[1] /*freader.result.split(',')[1]*/;
            //        $('#hiddenImg2').attr('src', 'data: image / jpeg; base64,' + $scope.thongTinKho.IMAGE2);
            //        $scope.lstB64.push($scope.thongTinKho.IMAGE2);
            //        $scope.CoDinhKem2 = true;
            //    }
            //    if (i === 2) {
            //        $scope.thongTinKho.IMAGE3 = $scope.stepsModel[2].split(',')[1];
            //        $('#hiddenImg3').attr('src', 'data: image / jpeg; base64,' + $scope.thongTinKho.IMAGE3);
            //        $scope.lstB64.push($scope.thongTinKho.IMAGE3);
            //        $scope.CoDinhKem3 = true;
            //    }
            //}
        });
    }
    ...
    $scope.ValueCount = (item, index) => {
        if (typeof item.SO_LUONG_TT == 'number' && typeof item.DON_GIA == 'number') {
            $scope.dsNhapKho[index].THANH_TIEN_TT = item.SO_LUONG_TT * item.DON_GIA;
        };
    }
