app.controller("KeHoachGiaoThangController", function ($scope, $uibModal, $ngConfirm, showToast, hideLoading) {
    $scope.canbo = [
    ...
    GetDanhMuc = () => {
        ...
        try {
            $.ajax({
                type: 'post',
                url: '/KeHoachXuat/GetDanhMucByAppCode',
                data: {},
    ...
    $scope.TimKiem = (num) => {
        ...
        $.ajax({
            type: 'post',
            url: '/KeHoachXuat/TimKiemKeHoach',
            data: {
    ...
    ThongTinHangHoa = (id) => {
        ...
        $.ajax({
            type: 'post',
            url: '/KeHoachXuat/DanhSachHangHoaTheoKeHoach',
            data: {
                idKeHoach: id
            },
            success: function (response) {
                if (response.Status === 200) {
                    ...
                    console.warn($scope.dsHangHoaDaChon);
                    console.warn($scope.dsChitietToanBo);
                    $scope.ShowChiTietHangHoa(0);
                }
                else {
                    toastr.error('Có lỗi xảy ra trong quá trình tìm kiếm!');
    ...
    $scope.LuuPhieu = () => {
        ...
        try {
            ...
            $scope.thongTinKh.APP_CODE = 'PHUONG_TIEN_BO';
            $scope.thongTinKh.LOAI_KH = 'XK';
            $scope.thongTinKh.NGAY_XUAT_DK_TU = moment($scope.thongTinKh.NGAY_XUAT_DK_TU_VW, 'DD/MM/YYYY').format('YYYYMMDD');
            $scope.thongTinKh.NGAY_XUAT_DK_DEN = moment($scope.thongTinKh.NGAY_XUAT_DK_DEN_VW, 'DD/MM/YYYY').format('YYYYMMDD');
            $scope.thongTinKh.NGAY_NHAN_DK = moment($scope.thongTinKh.NGAY_NHAN_DK_VW, 'DD/MM/YYYY').format('YYYYMMDD');
            $scope.thongTinKh.NGAY_KH = moment($scope.thongTinKh.NGAY_KH_VW, 'DD/MM/YYYY').format('YYYYMMDD');
            $scope.dsChitietToanBo.forEach(x => { x.ID_NGUON_KP = $scope.thongTinKh.ID_NGUON_KP, x.ID_NGUON_GOC = $scope.thongTinKh.ID_NGUON_GOC, x.ID_KE_HOACH = $scope.thongTinKh.ID_KE_HOACH });
            if ($scope.thongTinKh.ID_KE_HOACH != null) {
                if ($('#main-info').valid()) {
                    if ($scope.dsChitietToanBo.length > 0) {
                        $.ajax({
                            type: 'post',
                            url: '/KeHoachXuat/UpdateKeHoach',
                            data: {
                            ...
                            success: function (response) {
                                if (response.Status === 200) {
                                    toastr.success('Cập nhật thông tin xuất kho thành công!');
                                }
                                else {
                                    toastr.error('Có lỗi xảy ra trong quá trình cập nhật!');
                                }
                ...
                if ($('#main-info').valid()) {
                    if ($scope.dsChitietToanBo.length > 0) {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response.Status === 200) {
                                    toastr.success('Lưu thông tin xuất kho thành công!');
    ...
    GenerateRandomNumber = () => {
        ...
        var soPhieu = 'KHX' + moment(newDate).format('YYYYMMDDHHmm');
        return soPhieu;
    }
