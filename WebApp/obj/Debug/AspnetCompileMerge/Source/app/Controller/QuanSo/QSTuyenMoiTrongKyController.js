app.controller("QSTuyenMoiTrongKyController", function ($scope, $uibModal, $ngConfirm, showToast, hideLoading, $location) {
    //#region Khởi tạo
    $scope.treeLucLuong;
    $scope.treeCapBac;
    ...
    function onClickTCCBCS(event, treeId, treeNode, clickFlag) {
        ...
        if (treeNode !== undefined && treeNode.id !== undefined) {
            ...
            $scope.LoadDataTuyenMoiTrongKy($scope.DON_VI_ID);
            $scope.IsShowRight = true;
            $scope.$apply();
        }
        hideLoading();
    ...
    //#region QS Tuyển mới trong kỳ
    $scope.LoadDataTuyenMoiTrongKy = function (donViId) {
        $.ajax({
            type: 'post',
            url: '/QSTuyenMoiTrongKy/GetDanhSach',
