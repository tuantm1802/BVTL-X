...
namespace SyncBVTL.Push.Controllers.PA
{
    public class SyncDataController : BaseController
    {
        ...
        public async Task<BaseResult> GetDataFromAPI(ProcessModel model)
        {
            ...
            if (model.Active)
            {
                apiResult = await syncDataFromApi_SaveToDB.GetDataFromApi_SaveToDB(model.Url, model.Token, model.ReportId, model.MaDuAn, model.TableNames, model.Code, model.RawOrLabel);
            }
            return apiResult;
        }
