...
{
    public class KetQuaASSISTDA : IKetQuaASSISTDA
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public KetQuaASSISTPageModel GetItemById(int id)
        {
            var result = new KetQuaASSISTPageModel();

            var param = new List<SqlParameter>
                {
                    new SqlParameter("Id", id),
                };
            var resultPro = _DatabaseSql.ExecuteProcToList<KetQuaASSISTPageModel>(Constants.SP_KetQuaASSIST_Get_By_Id, param).ToList();
        ...
        /// <returns></returns>
        public List<KetQuaASSISTPageModel> GetAllByPage(ModelSearch modelSearch)
        {
            var result = new List<KetQuaASSISTPageModel>();
            try
            {
                var param = new List<SqlParameter>
                {
                ...
                result = _DatabaseSql.ExecuteProcToList<KetQuaASSISTPageModel>(Constants.SP_KetQuaASSIST_Get_By_Page, param).ToList();
            }
            catch (Exception ex)
            {
                var log = new BVTL_QT_LOG
                {
                    ControllerName = "KetQuaASSISTDA",
                    UserName = "",
                    DateLog = DateTime.Now,
                    Content = "Lấy danh sách kết quả ASSIST theo trang lỗi:" + ex.Message
                };
                db.BVTL_QT_LOG.Add(log);
                result = new List<KetQuaASSISTPageModel>();
            }
            return result;
        }
    }
