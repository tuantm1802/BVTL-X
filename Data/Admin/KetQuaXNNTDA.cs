...
{
    public class KetQuaXNNTDA : IKetQuaXNNTDA
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public KetQuaXNNTPageModel GetItemById(int id)
        {
            var result = new KetQuaXNNTPageModel();

            var param = new List<SqlParameter>
                {
                    new SqlParameter("Id", id),
                };
            var resultPro = _DatabaseSql.ExecuteProcToList<KetQuaXNNTPageModel>(Constants.SP_KetQuaXNNT_Get_By_Id, param).ToList();
        ...
        /// <returns></returns>
        public List<KetQuaXNNTPageModel> GetAllByPage(ModelSearch modelSearch)
        {
            var result = new List<KetQuaXNNTPageModel>();
            try
            {
                var param = new List<SqlParameter>
                {
                ...
                result = _DatabaseSql.ExecuteProcToList<KetQuaXNNTPageModel>(Constants.SP_KetQuaXNNT_Get_By_Page, param).ToList();
            }
            catch (Exception ex)
            {
                var log = new BVTL_QT_LOG
                {
                    ControllerName = "KetQuaXNNTDA",
                    UserName = "",
                    DateLog = DateTime.Now,
                    Content = "Lấy danh sách kết quả HIV theo trang lỗi:" + ex.Message
                };
                db.BVTL_QT_LOG.Add(log);
                result = new List<KetQuaXNNTPageModel>();
            }
            return result;
        }
    }
