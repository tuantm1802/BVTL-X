...
{
    public class ChuyenGuiDichVuDA : IChuyenGuiDichVuDA
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public ChuyenGuiDichVuPageModel GetItemById(int id)
        {
            var result = new ChuyenGuiDichVuPageModel();

            var param = new List<SqlParameter>
                {
                    new SqlParameter("Id", id),
                };
           var resultPro = _DatabaseSql.ExecuteProcToList<ChuyenGuiDichVuPageModel>(Constants.SP_ChuyenGuiDichVu_Get_By_Id, param).ToList();
        ...
        /// <returns></returns>
        public List<ChuyenGuiDichVuPageModel> GetAllByPage(ModelSearch modelSearch)
        {
            var result = new List<ChuyenGuiDichVuPageModel>();
            try
            {
                var param = new List<SqlParameter>
                {
                ...
                result = _DatabaseSql.ExecuteProcToList<ChuyenGuiDichVuPageModel>(Constants.SP_ChuyenGuiDichVu_Get_By_Page, param).ToList();
            }
            catch (Exception ex)
            {
                var log = new BVTL_QT_LOG
                {
                    ControllerName = "ChuyenGuiDichVuDA",
                    UserName = "",
                    DateLog = DateTime.Now,
                    Content = "Lấy danh sách chuyển gửi dịch vụ theo trang lỗi:" + ex.Message
                };
                db.BVTL_QT_LOG.Add(log);
                result = new List<ChuyenGuiDichVuPageModel>();
            }
            return result;
        }
    }
